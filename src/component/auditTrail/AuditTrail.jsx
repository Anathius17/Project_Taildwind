...
const AuditTrail = () => {
  ...
  const [detailModulParam, setDetailModulParam] = useState("");
  const [viewDetail, setViewDetail] = useState();
  const [dataListLog, setDataListLog] = useState([]);
  // Dapatkan data sesi
  ...
  const getListModul = async () => {
    try {
      const listUser = await axios.get(
        "http://116.206.196.65:30998/skyaudittrail/audit/list_modul",
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const getListLog = async () => {
    try {
      const userRoleDetail = await axios.post(
        "http://localhost:30998/skyaudittrail/audit/list_log",
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      setDataListLog(dataUserRoleDetailApi);
  ...
  const getViewDetail = async () => {
    try {
      const listViewDetail = await axios.post(
        "http://116.206.196.65:30998/skyaudittrail/audit/view_detail/" +
          viewDetail,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const cekData = listViewDetail.data.data.map((e) => {
        return e;
      });
      console.log(listViewDetail);
      setViewDetail(cekData[0]);
    } catch (errorUser) {
                  ...
                  >
                    ...
                    <td className="py-3 px-6 text-left  whitespace-nowrap ">
                      <button
                        ...
                        onClick={() => openModal(user.usruserid)}
                      >
                        <svg
